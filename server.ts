  ...
  app.get<{ resourceId: number }, {}, {}>(
    ...
    async (req, res) => {
      ...
      if (dbres1.rowCount === 0) {
        ...
        const query2 =
          "SELECT feedback.id, feedback.liked, feedback.comment, users.name, users.id as user_id FROM feedback JOIN users ON feedback.user_id = users.id WHERE resource_id = $1";
